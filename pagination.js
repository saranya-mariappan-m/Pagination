// Raw data for buiding Table Content.
...
function generateTableHead(table, data) {
    ...
    for (let key of data) {
        ...
        th.style.backgroundColor = "#E7E9EB";
...
const generateTable = function (tbody, data) {
...
// Function to build the pager for PAGINATION.
...
const buildPager = () => {
    ...
    wrapper.setAttribute('id', 'buttons');
    wrapper.classList.add('d-flex', 'justify-content-center');

    const first = document.createElement('button');
    first.innerText = 'First';
    first.classList.add('flex-items');

    const preBtn = document.createElement('button');
    preBtn.innerText = 'Previous';
    preBtn.classList.add('flex-items');

    const next = document.createElement('button');
    next.innerText = 'Next';
    next.classList.add('flex-items');

    const last = document.createElement('button');
    last.innerText = 'Last';
    last.classList.add('flex-items');

    wrapper.append(first, preBtn);
    var attachTo = preBtn;

    for (let i = 20; i <= data.length; i = i + 10) {

        const pagers = document.createElement('button');
        const currentPage = i / 10;
        pagers.setAttribute('id', 'page' + i);
        pagers.setAttribute('data-page', currentPage);
        pagers.classList.add('flex-items');


        pagers.innerText = currentPage;
        wrapper.append(pagers);
        attachTo = pagers;

    }
    wrapper.append(next, last);
    document.body.append(wrapper);
    const flexItems = document.querySelectorAll('.flex-items');
    flexItems.forEach(flexItem => {
        flexItem.style.width = 'auto';
        flexItem.style.padding = '10px 20px';
        flexItem.style.border = '1px solid #ddd';
        flexItem.style.borderRight = '0px';
        flexItem.style.marginLeft = '-1px';
        flexItem.style.fontFamily = 'sans-serif';
        flexItem.style.color = 'dodgerblue';
        if (currPage == flexItem.dataset.page) {
            flexItem.style.backgroundColor = "blue";
            flexItem.style.color = "#FFFFFF";
        } else {
            flexItem.style.backgroundColor = 'transparent';
        }

        flexItem.addEventListener("mouseover", function (e) {
            this.style.cursor = 'Pointer';
        });
        flexItem.addEventListener("click", function (e) {
            const targetID = e.target.id;
            if (targetID.includes('page')) {
                currPage = parseInt(this.dataset.page);
                loadResults();
            }
    ...
    first.style.borderRadius = '5px 0 0 5px';
    last.style.borderRadius = '0 5px 5px 0';
    last.style.borderRight = '1px solid #ddd';

    if (currPage <= 1) {
        preBtn.disabled = true;
        preBtn.style.color = '#1010104d';
        first.disabled = true;
        first.style.color = '#1010104d';
    } else {
    ...
    if (currPage >= 10) {
        next.disabled = true;
        next.style.color = '#1010104d';
        last.disabled = true;
        last.style.color = '#1010104d';
    } else {
        next.disabled = false;
...
// Function to load the table data as per pager.
...
const loadResults = () => {
    ...
    pelement.innerText = 'Pagination for the Table JSON Data';
    ...
    document.getElementById('tableWrapper').appendChild(tableElement);
    ...
    data.slice(startIndex, endIndex).forEach((element) => {
        generateTable(tbody, element);
    });
    buildPager();

}

// Function call to display page content.
